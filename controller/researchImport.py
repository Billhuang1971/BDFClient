...
from view.researchImport import ResearchImportView
from view.researchImport import AddFormView
import pyedflib
import re
import datetime

from view.researchImport_form.loadingform import LoadingDialog
from view.researchImport_form.label_table import LabelTypeTableWidget
from view.researchImport_form.mapping_table import MappingView
from view.researchImport_form.patient_table import PatientTableWidget
from view.researchImport_form.doctor_table import DoctorTableWidget
from view.progressBarView import ProgressBarView


class researchImportController(QWidget):
    # 上传信号
    ...
    def __init__(self, client, cAppUtil, mainMenubar=None):
        ...
        # 存放提取到的type映射和sample信息的上级目录
        self.exdir_path = os.path.join(self.root_path, 'upload', 'Extract')
        # 存放提取出的type映射和sample信息目录
        ...
        self.client.getPatientCheckInfoResSig1.connect(self.getPatientCheckInfoRes)
        self.client.delPatientCheckInfoResSig1.connect(self.delPatientCheckInfoRes)
        self.client.addCheckInfoResSig1.connect(self.addCheckInfoRes)
        self.client.checkConfigResSig1.connect(self.checkConfigRes)
        self.client.getUserConfigResSig1.connect(self.getUserConfigRes)
        self.client.makeFileNameResSig1.connect(self.makeFileNameRes)
        self.client.writeEEGResSig1.connect(self.writeEEGRes)
        self.client.updateCheckInfoResSig1.connect(self.updateCheckInfoRes)
        self.client.getFileInfoResSig1.connect(self.getFileInfoRes)
        self.client.delFileInfoResSig1.connect(self.delFileInfoRes)
        self.client.getChoosePatientInfoResSig1.connect(self.get_choose_patient_infoRes)
        ...
        # 设置选择标注类型的绑定事件
        self.view.ui.selectLabelTypeBtn.clicked.connect(self.onChoose_LabelType)
    ...
    def addUserFolder(self):
        ...
        # 拼接提取文件夹及提取文件
        self.extract_path = os.path.join(self.exdir_path, account)
        if not os.path.exists(self.extract_path):
            os.makedirs(self.extract_path)
        self.mapping_path = os.path.join(self.extract_path, 'mapping.json')
        self.sample_path = os.path.join(self.extract_path, 'sample.json')
    ...
    def long_task_with_dialog(self, post_callback=None):
        self.loading_dialog = LoadingDialog("正在转换 DAT 文件，请稍候...", self)
    ...
    def on_btnAddFile_clicked(self, row):
        ...
        if ok:
            ...
            if file_extension == '.dat':
                ...
                QMessageBox.information(self, "不支持的文件类型", "请上传有效的脑电文件格式 (.dat).")
                return
    ...
    def checkBasicConfig(self):
        ...
        self.client.checkConfig1(REQmsg)
    ...
    def getUserConfig(self):
        ...
        self.client.getUserConfig1(REQmsg)
    ...
    def on_btnRemove_clicked(self, row):
        ...
        if not self.cAppUtil.isNull(self.dir_path):
            ...
            if bdfFileNameList:
                ...
                if match and int(match.group(1)) == check_id:
    ...
    def isJsonListEmpty(self,file_path):
        ...
        try:
            ...
            return True  # 解析失败（如空文件）也算空

    def removeMapping(self, target_check_number):
        try:
            with open(self.mapping_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            # 过滤掉匹配的记录
            new_data = [entry for entry in data if entry.get("check_number") != target_check_number]
            # 写回文件
            with open(self.mapping_path, 'w', encoding='utf-8') as f:
                json.dump(new_data, f, indent=4, ensure_ascii=False)
            print(f"已删除 check_number = {target_check_number} 的记录")
        except Exception as e:
            print("删除失败：", e)
    ...
    def on_btnUploadStart_clicked(self):
        ...
        if self.cAppUtil.isNull(self.dir_path):
            ...
            if not self.cAppUtil.isNull(self.queue_file_path):
                ...
                if self.is_processing:
                    print("上一个文件正在处理中...")
                    return
                self.client.makeFileName1(REQmsg)
        ...
        # client_root/upload/EEG不为空（被中断上传的文件只可能有一个）
            ...
            if ret[0] == '1':
                ...
                self.client.delFileInfo1(REQmsg)
                # 打开所有功能键
                self.view.ui.groupBox_3.setEnabled(True)
                self.view.ui.groupBox_4.setEnabled(True)
                self.view.ui.groupBox.setEnabled(True)
    ...
    def getFileInfo(self, value=''):
        ...
        uid = self.client.tUser[0]
        REQmsg = [account, uid, value]
        self.client.getFileInfo1(REQmsg)
    ...
    # 点击选择标注类型
    def onChoose_LabelType(self):
        # 先检查是否输入检查单号
        check_num = self.view.ui.check_num.text()
        if check_num == '':
    ...
    def get_labeltype_info(self, flag='1', key_word='', key_value=''):
        account = self.client.tUser[1]
        if flag == '1':
            ...
            REQmsg = [account, flag, self.perPageNum, self.labeltype_start, key_word, key_value]
        # 你需要定义对应的 client.getChooseLabeltypeInfo1 方法
        self.client.getChooseLabelTypeInfo1(REQmsg)


    def get_choose_labeltype_infoRes(self, REPData):
        if REPData[0] == '1':
            labeltype_info = REPData[3]
            self.labeltype_info = labeltype_info
            self.labeltype_totalNum = len(labeltype_info)

            # 创建视图组件实例
            self.labeltype_view = LabelTypeTableWidget()

            # 填充表格
            self.labeltype_view.set_table_data(self.labeltype_info)

            self.labeltype_view.confirm_btn.clicked.connect(self.confirm_mappings)
            # 显示界面
    ...
    def confirm_mappings(self):
        ...
        if not field_configs:
            QMessageBox.warning(None, "错误", "请至少填写一个字段", QMessageBox.Yes)
            return

        added = 0
        for config_group in field_configs:
            check_number = config_group['check_number']
            configs = config_group['configs']

            # 保存到 JSON 文件
            self.save_to_json(check_number, configs)
            added += 1
    ...
    def save_to_json(self, check_number, configs):
        ...
        # 查找是否已经有相同 check_number 的条目
        existing_entry = next((entry for entry in data if entry["check_number"] == check_number), None)

        if existing_entry:
            # 如果已经有此 check_number，则将新配置项添加到 configs 中
            existing_entry["configs"].extend(configs)
        else:
            # 否则，新增一个 check_number 条目
            data.append({
                "check_number": check_number,
                "configs": configs
            })
    ...
    def delete_from_json(self, check_number):
        ...
        # 过滤掉目标 check_number 对应的条目
        new_data = [entry for entry in data if entry.get("check_number") != check_number]
    ...
    def load_from_json(self,check_number: str):
        if not os.path.exists(self.mapping_path):
            return {"错误": "JSON 文件不存在"}
        ...
        try:
            ...
            for entry in all_data:
                if entry.get("check_number") == str(check_number):
                    return entry["configs"]

            return {"提示": f"未找到 check_number: {check_number}"}

        except Exception as e:
            return {"错误": str(e)}


    def on_btnViewMapping_clicked(self,row):
        check_number = self.view.data[row][5]
        mapping = self.load_from_json(check_number)
        self.mapping_window = MappingView(mapping)
    ...
    def get_choose_patient_info(self, flag='1', key_word='', key_value=''):
        ...
        if flag == '1':
            account = self.client.tUser[1]
            REQmsg = [account, flag, self.perPageNum]
            self.client.getChoosePatientInfo1(REQmsg)
        # 病人信息没有条件的翻页
        elif flag == '2':
            account = self.client.tUser[1]
        ...
        try:
            if REPData[0] == '1':
                ...
                if flag == '1':
                    ...
                    self.view.tableWidget_patient.setWindowTitle("被试相关信息")
                    # 初始化表格信息
    ...
    def get_choose_doctor_info(self, flag='1', key_word='', key_value=''):
        ...
        if flag == '1':
            account = self.client.tUser[1]
            REQmsg = [account, flag, self.perPageNum]
            self.client.getChooseDoctorInfo1(REQmsg)
        # 医生信息没有条件的翻页
        elif flag == '2':
            account = self.client.tUser[1]
        ...
        try:
            if REPData[0] == '1':
                ...
                if flag == '1':
                    ...
                    self.view.tableWidget_doctor.setWindowTitle("研究员相关信息")
                    # 初始化表格信息
    ...
    def on_btnCancelAdd_clicked(self):
        ...
        self.view.ui.patientBtn.setFont(QFont("Agency FB", 16))
        self.view.ui.pdoctorBtn.setText('选择开单研究员')
    ...
    def convert_dat_to_bdf(self):
        ...
        try:
            ...
            prev_state = None
            events = []
            for i, state in enumerate(raw_states.T):
                if prev_state is None or not np.array_equal(state, prev_state):
                    events.append((i / sampling_rate, f"StateChange:{state}"))
                prev_state = state
            # 确保数据是 μV 单位  =》数据已然是 μV 单位 ，无需转换
            # signal_data *= 1e3  # 从 V 转换为 μV

            # 计算 block size
            block_size = sampling_rate  # 1秒的数据作为一个 block
            n_samples = signal_data.shape[0]

            # 确保数据长度是 block_size 的整数倍，不足则填充
            remainder = n_samples % block_size
            if remainder != 0:
                pad_size = block_size - remainder
                signal_data = np.pad(signal_data, ((0, pad_size), (0, 0)), mode='constant', constant_values=0)
                n_samples = signal_data.shape[0]  # 更新样本数

            # 创建 .bdf 文件
            with pyedflib.EdfWriter(self.convert_filepath, n_channels, file_type=pyedflib.FILETYPE_BDFPLUS) as bdf_file:
                # 设置通道信息
                channel_info = []
                for i in range(n_channels):
                    ch_dict = {
                        ...
                        'dimension': 'uV',  # 单位为 μV
                        'sample_frequency': sampling_rate,
                        'physical_min': signal_data[:, i].min(),
                        'physical_max': signal_data[:, i].max(),
                        'digital_min': -8388608,  # BDF 格式要求 24-bit integer
                        'digital_max': 8388607,
                        'transducer': '',
                        'prefilter': ''
                    }
                ...
                bdf_file.setSignalHeaders(channel_info)

                # 按 block_size 写入数据
                for start in range(0, n_samples, block_size):
                    block = signal_data[start:start + block_size].T  # 形状应为 (n_channels, block_size)
                    bdf_file.writeSamples(block)

                # 写入事件标记
                for timestamp, event in events:
                    bdf_file.writeAnnotation(timestamp, 0, event)

                bdf_file.close()
        except Exception as e:
            print('Error during DAT to BDF conversion:', e)
    ...
    def exit(self):
        self.client.getPatientCheckInfoResSig1.disconnect()
        self.client.delPatientCheckInfoResSig1.disconnect()
        self.client.addCheckInfoResSig1.disconnect()
        self.client.checkConfigResSig1.disconnect()
        self.client.getUserConfigResSig1.disconnect()
        self.client.makeFileNameResSig1.disconnect()
        self.client.writeEEGResSig1.disconnect()
        self.client.updateCheckInfoResSig1.disconnect()
        self.client.getFileInfoResSig1.disconnect()
        self.client.delFileInfoResSig1.disconnect()
        self.client.getChoosePatientInfoResSig1.disconnect()
