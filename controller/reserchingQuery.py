...
class reserchingQueryController(QWidget):
    ...
    def __init__(self, appUtil=None, Widget=None, client=None, mainMenubar=None, mainLayout=None):
        ...
        self.client.rgQ_get_type_info(msg)
    ...
    def init_prentryView(self):
        ...
        self.view.ui.btnSignInfo.clicked.connect(partial(self.on_clicked_label_commit,'notqualified'))
        self.view.ui.btnSignInfo.setText('任务不合格')

        self.view.ui.nextPatientBtn.clicked.connect(self.on_btnNextPatient_clicked)
        self.view.ui.nextPatientBtn.setText('返 回')

        self.view.ui.nextFileBtn.setText('任务合格')
        self.view.ui.nextFileBtn.clicked.connect(partial(self.on_clicked_label_commit,'qualified'))
    ...
    def rg_pagingRes(self, REPData):
        ...
        self.diagListView.show()
        self.diagListView.init_table(self.diags_viewInfo,self.userNamesDict, self.paitentNamesDict,self.themeDict,
                                     self.on_clicked_manual_query,self.on_clicked_theme_commit, self.curPageIndex,
                                     self.curPageMax)
    ...
    def get_labelsRes(self, REPData):
        ...
        self.diagListView.show()
        self.diagListView.init_table(self.diags_viewInfo, self.userNamesDict,self.paitentNamesDict,self.themeDict,
                                     self.on_clicked_manual_query,self.on_clicked_theme_commit,
                                     self.curPageIndex,self.curPageMax)
    ...
    def on_clicked_label_commit(self,stats):
    ...
    def on_clicked_manual_query(self, diags_viewInfo,patient_name):
        ...
        try:
            self.uid=diags_viewInfo[12]
            msg = [self.check_id, self.file_id, self.patient_id, self.theme_id,diags_viewInfo[12],diags_viewInfo[3]]
            self.client.rgQ_openEEGFile(msg)
    ...
    def init_SampleList(self):
        ...
        self.client.rgQ_init_SampleList(msg)
    ...
    def disp_SampleList_ReadInitData(self):
        ...
        self.client.rgQ_load_dataDynamical(msg)
    ...
    def handle_pick_event(self, event):
        if self.task_state is None or self.task_state !='labelled':
            return
    ...
    def act_dispatch(self, type):
        ...
        # 增加样本
            ...
            QMessageBox.information(self.view, '不能添加 ', "无效选择")
    ...
    def update_sample(self, type):
        ...
        try:
                ...
                self.client.rgQ_update_sampleInfo15(msg)
    ...
    def del_sample(self):
        ...
        msg = [self.check_id, self.file_id, start,pick_channel, end, uid, type_id, self.theme_id]
        self.client.rgQ_del_sample19(msg)
    ...
    def insert_update_state(self, type):
        ...
        if self.state_picked is not None and self.state_right is None:
            ...
            self.client.rgQ_update_sampleInfo_state(msg)
        # 插入状态
        else:
            begin, end = self.state_left, self.state_right
            status = (pick_channel, begin, end, uid, type_id, type_name)
            msg = [self.check_id, self.file_id, begin, pick_channel, end, uid, type_id, self.theme_id]
            QMessageBox.information(self, '添加操作', "不能添加操作")
            #self.client.rgQ_add_sampleInfo_state(msg)
    ...
    def del_state(self):
        if self.state_picked is not None:
            ...
            pick_channel, start, end, uid, type_id = label[0], label[1], label[2], label[3], label[4]
            msg = [self.check_id, self.file_id, start, pick_channel, end,uid, type_id, self.theme_id]
            self.client.rgQ_del_sampleInfo_state(msg)
    ...
    def timeChange(self):
        ...
        msg = [self.check_id, self.file_id, REQdata]
        self.client.rgQ_load_dataDynamical10(msg)
    ...
    def del_label(self):
        ...
        self.idx = idx
        msg = [self.check_id,self.file_id, start, pick_channel,end, uid, type_id, self.theme_id]
        self.client.rgQ_del_sampleInfo13(msg)
        return
    ...
    def update_label(self, type):
        ...
        try:
                ...
                msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id, self.theme_id]
                self.client.rgQ_update_sampleInfo12(msg)
