...
from view.manualQuery import ManualView, BdfSettingView

from view.manualQuery import SettingView
from view.manualQuery import PrentryView
from view.manualQuery import sign_InfoView
from view.manualQuery import diag_diagnosedListView
...
class manualQueryController(QWidget):
    ...
    def __init__(self, appUtil=None, Widget=None, client=None, mainMenubar=None, mainLayout=None):
        ...
        self.client.mq_get_type_info(msg)
    ...
    def mq_get_type_infoRes(self, REPData):
        if REPData[0] == '0':
    ...
    def init_prentryView(self):
        ...
        self.diagListView = diag_diagnosedListView()
        self.diagListView.page_control_signal.connect(self.mq_paging)

        self.mq_get_diags_Diagnosed()
        self.view.hide()
    ...
    def on_clicked_manual_query(self, diags_viewInfo,patient_name):
        ...
        msg = [self.check_id]
        self.client.mq_get_fileNameByIdDate(msg)
        self.view.ui.btnSignInfo.setText(f"[医生{self.userNamesDict.get(diags_viewInfo[2])},单号{diags_viewInfo[-2]}]诊断信息")
        self.returnTo=0
        #self.view.show()
    ...
    def on_btnConfirm_clicked(self):
        try:
            ...
            self.client.mq_openEEGFile(msg)
            self.prentryView.hide()
    ...
    def mq_openEEGFileRes(self, REPData):
        if REPData[0] == '0':
            ...
            self.view.show_patient_info(self.patientInfo, self.file_name,
                                                 ...
                                                 self.end_time)
            self.mq_disp_SampleList_ReadInitData()
    ...
    def mq_load_dataDynamical(self):
        size = 60
    ...
    #绘图设置【??仅请求数据查询，未返回结果】
    def mq_init_SampleListRes(self, REPData):
        if REPData[0] == '0':
    ...
    def mq_init_SampleList(self):
        ...
        self.client.mq_init_SampleList(msg)

    def mq_disp_SampleList_ReadInitData(self):
        self.disp_SampleList()
        REQdata=self.mq_load_dataDynamical()
        msg = [self.check_id, self.file_id, REQdata]
        self.client.mq_load_dataDynamical(msg)

    def mq_load_dataDynamicalRes(self, REPData):
    ...
    def on_btnSignInfo_clicked(self):
        if self.sign_InfoView is None:
          ...
          self.client.mq_get_diag(msg)
        else:
          try:
            print("on_btnSignInfo_clicked try")
    ...
    #诊断标注/提取诊断信息回调
    def mq_get_diagRes(self, REPData):
        self.view.ui.btnSignInfo.setEnabled(True)
        if REPData[0] == '0':
        ...
        if self.diag is None or len(self.diag) == 0:
          ...
          self.sign_InfoView.ui.summary_textEdit.setText("")
            ...
            self.sign_InfoView.ui.d_user_lineEdit_3.setText(self.userNamesDict.get(self.diag[0][2]))
