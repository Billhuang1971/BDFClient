...
class reserchingController(QWidget):
    ...
    def __init__(self, appUtil=None, Widget=None, client=None, mainMenubar=None, mainLayout=None):
        ...
        self.client.rg_openEEGFileResSig.connect(self.openEEGFileRes)
        self.client.rg_load_dataDynamicalResSig.connect(self.load_dataDynamicalRes)
        self.client.rg_update_sampleInfo12ResSig.connect(self.update_sampleInfo12Res)
        self.client.rg_del_sampleInfo13ResSig.connect(self.del_sampleInfo13Res)
        self.client.rg_add_sampleInfo14ResSig.connect(self.update_sample1415Res)
        self.client.rg_update_sampleInfo15ResSig.connect(self.update_sample1415Res)
        self.client.rg_insert_sample17ResSig.connect(self.insert_sample17Res)
        self.client.rg_init_SampleListResSig.connect(self.init_SampleListRes)
        self.client.rg_del_sample19ResSig.connect(self.del_sample19Res)
        self.client.rg_add_sampleInfo_stateResSig.connect(self.add_sampleInfo_stateRes)
        self.client.rg_update_sampleInfo_stateResSig.connect(self.add_sampleInfo_stateRes)
        self.client.rg_del_sampleInfo_stateResSig.connect(self.del_sampleInfo_stateRes)
    ...
    def exit(self):
        ...
        self.client.rg_openEEGFileResSig.disconnect()
        self.client.rg_load_dataDynamicalResSig.disconnect()
        self.client.rg_update_sampleInfo12ResSig.disconnect()
        self.client.rg_del_sampleInfo13ResSig.disconnect()
        self.client.rg_add_sampleInfo14ResSig.disconnect()
        self.client.rg_update_sampleInfo15ResSig.disconnect()
        self.client.rg_insert_sample17ResSig.disconnect()
        self.client.rg_init_SampleListResSig.disconnect()
        self.client.rg_del_sample19ResSig.disconnect()
        self.client.rg_add_sampleInfo_stateResSig.disconnect()
        self.client.rg_update_sampleInfo_stateResSig.disconnect()
        self.client.rg_del_sampleInfo_stateResSig.disconnect()
    ...
    def init_prentryView(self):
        ...
        self.view.ui.btnSignInfo.setText('完成当前标注任务')
    ...
    # 处理客户端传回的提取取诊断信息
    def get_notlabelsRes(self, REPData):
    ...
    def on_clicked_manual_query(self, diags_viewInfo,patient_name):
        ...
        if self.task_state is None or self.task_state not in ('notStarted', 'labelling'):
           self.popMenu2.setDisabled(True)
           self.popMenu1.setDisabled(True)
           self.Menu.setDisabled(True)
           self.view.ui.btnSignInfo.hide()
        else:
           self.popMenu2.setEnabled(True)
           self.popMenu1.setEnabled(True)
           self.Menu.setEnabled(True)
           self.view.ui.btnSignInfo.show()
        ...
        self.page = ['file_name']
        try:
            msg = [self.check_id, self.file_id, self.patient_id, self.theme_id,self.client.tUser[0],diags_viewInfo[3]]
            self.client.rg_openEEGFile(msg)
    ...
    def init_SampleList(self):
        ...
        user_names ="('{}')".format(self.uid)
        msg = [self.check_id, self.file_id, channels,type_names, user_names, status_show, self.theme_id]
        self.client.rg_init_SampleList(msg)
    ...
    def disp_SampleList_ReadInitData(self):
        ...
        self.client.rg_load_dataDynamical(msg)
    ...
    def insert_sample(self, type):
        ...
        try:
            ...
            msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id, self.theme_id]
            self.client.rg_insert_sample17(msg)
    ...
    def update_sample(self, type):
        ...
        if label[3] == self.User[0]:
            try:
                msg = [ self.check_id, self.file_id, start,pick_channel, end, uid, type_id, self.theme_id]
                self.client.rg_update_sampleInfo15(msg)
            except Exception as result:
                QMessageBox.information(self.view, '修改失败', "失败原因: %s" % result)
        else:
            try:
                msg = [self.check_id,  self.file_id, start,pick_channel, end, uid, type_id, self.theme_id]
                self.client.rg_add_sampleInfo14(msg)
            except Exception as result:
    ...
    def del_sample(self):
        ...
        if self.User[0] == label[3]:
            msg = [self.check_id, self.file_id, start,pick_channel, end, uid, type_id, self.theme_id]
            self.client.rg_del_sample19(msg)
        else:
    ...
    def insert_update_state(self, type):
        ...
        if self.state_picked is not None and self.state_right is None:
            ...
            msg = [self.check_id, self.file_id, begin, pick_channel, end,uid, type_id, self.theme_id]

            self.client.rg_update_sampleInfo_state(msg)
        # 插入状态
        else:
            ...
            self.client.rg_add_sampleInfo_state(msg)
    ...
    def del_state(self):
        if self.state_picked is not None:
            ...
            pick_channel, start, end, uid, type_id = label[0], label[1], label[2], label[3], label[4]
            msg = [self.check_id, self.file_id, start, pick_channel, end,uid, type_id, self.theme_id]
            self.client.rg_del_sampleInfo_state(msg)
    ...
    def timeChange(self):
        ...
        msg = [self.check_id, self.file_id, REQdata]
        self.client.rg_load_dataDynamical10(msg)
    ...
    def del_label(self):
        ...
        if self.User[0] == sample[3]:
            ...
            self.idx = idx
            msg = [self.check_id,self.file_id, start, pick_channel,end, uid, type_id, self.theme_id]
            self.client.rg_del_sampleInfo13(msg)
    ...
    def update_label(self, type):
        ...
        if sample[3] == self.User[0]:
            try:
                ...
                msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id, self.theme_id]
                self.client.rg_update_sampleInfo12(msg)
            ...
            try:
                msg = [self.check_id, self.file_id, start,pick_channel, end, uid, type_id, self.theme_id]
                self.client.rg_add_sampleInfo11(msg)
