...
from view.diagAssess import SettingView
...
class diagAssessController(QWidget):
    ...
    def __init__(self, appUtil=None, Widget=None, client=None, mainMenubar=None, mainLayout=None):
        ...
        self.client.da_get_type_infoResSig.connect(self.get_type_infoRes)
        self.client.da_get_contentsResSig.connect(self.get_contentsRes)
        self.client.da_openEEGFileResSig.connect(self.openEEGFileRes)
        self.client.da_load_dataDynamicalResSig.connect(self.load_dataDynamicalRes)
        self.client.da_update_sampleInfo12ResSig.connect(self.update_sampleInfo12Res)
        self.client.da_del_sampleInfo13ResSig.connect(self.del_sampleInfo13Res)
        self.client.da_add_sampleInfo14ResSig.connect(self.update_sample1415Res)
        self.client.da_update_sampleInfo15ResSig.connect(self.update_sample1415Res)
        self.client.da_insert_sample17ResSig.connect(self.insert_sample17Res)
        self.client.da_init_SampleListResSig.connect(self.init_SampleListRes)
        self.client.da_del_sample19ResSig.connect(self.del_sample19Res)
        self.client.da_add_sampleInfo_stateResSig.connect(self.add_sampleInfo_stateRes)
        self.client.da_update_sampleInfo_stateResSig.connect(self.add_sampleInfo_stateRes)
        self.client.da_del_sampleInfo_stateResSig.connect(self.del_sampleInfo_stateRes)
        ...
        self.client.da_get_type_info(msg)
    ...
    def exit(self):
        ...
        self.client.da_get_type_infoResSig.disconnect()
        self.client.da_get_contentsResSig.disconnect()
        self.client.da_openEEGFileResSig.disconnect()
        self.client.da_load_dataDynamicalResSig.disconnect()
        self.client.da_update_sampleInfo12ResSig.disconnect()

        self.client.da_del_sampleInfo13ResSig.disconnect()
        self.client.da_add_sampleInfo14ResSig.disconnect()
        self.client.da_update_sampleInfo15ResSig.disconnect()
        self.client.da_insert_sample17ResSig.disconnect()
        self.client.da_init_SampleListResSig.disconnect()

        self.client.da_del_sample19ResSig.disconnect()
        self.client.da_add_sampleInfo_stateResSig.disconnect()
        self.client.da_update_sampleInfo_stateResSig.disconnect()
        self.client.da_del_sampleInfo_stateResSig.disconnect()
    ...
    def init_prentryView(self):
        ...
        #self.view.ui.btnRuler.setDisabled(True)
        ...
        #self.view.ui.btnStateAnnotate.setDisabled(True)
        ...
        self.view.ui.nextPatientBtn.setText("选择课堂")
        self.view.ui.nextFileBtn.setText("选择课程内容")
        self.view.ui.nextPatientBtn.clicked.connect(self.on_btnNextContent_clicked)
        self.view.ui.nextFileBtn.clicked.connect(self.on_btnNextStudent_clicked)
    ...
    def on_btnNextStudent_clicked(self):
    ...
    def on_btnNextContent_clicked(self):
      self.stopRolling()
      self.view.ui.nextFileBtn.setDisabled(True)
      self.view.ui.nextPatientBtn.setDisabled(True)
      self.view.ui.btnSignInfo.setDisabled(True)
    ...
    def get_contentsRes(self, REPData):
        ...
        self.diagListView.show()
        if self.client.tUser[10]:
           self.diagListView.init_table(self.diags_viewInfo,
                              self.on_clicked_manual_query,self.on_btnDelClass_clicked)
        else:
           self.diagListView.init_table(self.diags_viewInfo,self.on_clicked_manual_query, None)
    ...
    def on_tableWidget_itemClicked2(self):
        ...
        self.view.ui.btnSignInfo.setText(
            f"[医生{self.userNamesDict.get(self.classContentsInfo[row][2])},单号{self.classContentsInfo[row][3]}]诊断信息")
    ...
    def openEEGFileRes(self, REPData):
        if REPData[0] == '0':
            ...
            self.d_sample_list = REPData[12]
            self.qsum= REPData[13]
            self.asum = REPData[14]
            self.studentInfo = REPData[15]

            self.d_sample_listDict = {}
            if self.d_sample_list is not None:
                for u in self.d_sample_list:
                    skey=str(u[0])+'-'+str(u[1])+'-'+str(u[2])
                    self.d_sample_listDict.setdefault(skey, u[5])
            self.montage = 'Default'
            self.mont_idx = 0
            self.begin = 0
            self.index = 0

            self.view.show_patient_info(self.patientInfo, f'{self.studentInfo[0][4]}-{self.studentInfo[0][5]}',
                                        self.qsum, self.asum,self.studentInfo[0][3])
    ...
    def init_SampleListRes(self, REPData):
        ...
        self.widget_from_main.setText(
            '绘图长度(s): {}    移动长度(s): {}    灵敏度(µV/mm)：{}    参考方案：{}'.format(self.paint_length,
                                                                                       ...
                                                                                       self.sensitivity,
                                                                                       self.montage))
        self.paint(self.data, self.begin, self.end, channels=self.channels, updateY=True)
    ...
    def init_SampleList(self):
        ...
        msg = [self.check_id, self.file_id, channels, type_names, user_names, status_show,self.class_id,self.stu_id]
        self.client.da_init_SampleList(msg)
    ...
    def disp_SampleList_ReadInitData(self):
        ...
        self.client.da_load_dataDynamical(msg)
    ...
    def disp_SampleList(self):
        ...
        for r in range(row_num):
            for c in range(col_num):
                ...
                skey = str(self.sample_list[r][0]) + '-' + str(self.sample_list[r][1]) + '-' + str(self.sample_list[r][2])
                svalue = self.d_sample_listDict.get(skey)
                item.setToolTip(svalue)
                font = item.font()
                font.setPointSize(8)
                item.setFont(font)
                if svalue== self.sample_list[r][5]:
    ...
    def paint_status(self, status_info, Update=False):
        ...
        for status in status_info:
            if status[4] is None or status[4]=='None':
                description = '[测试]'
                color='blue'
            else:
    ...
    def create_axes(self, begin, end, updateY, btn) -> None:
        ...
        self.axes.grid(ls='-', lw=0.05, axis='x')
    ...
    def insert_sample(self, type):
        QMessageBox.information(self.view, '不能添加', "不能操作")
        return
        color = 'blue'
        ...
        try:
            ...
            self.client.da_insert_sample17(msg)
    ...
    def update_sample(self, type):
        ...
        pick_channel, start, end,uid,suid = label[0], label[1], label[2], label[3], label[6]
        ...
        try:
                msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id,self.class_id,suid]
                self.client.da_update_sampleInfo15(msg)
        except Exception as result:
                QMessageBox.information(self.view, '添加失败', "失败原因: %s" % result)
        return
    ...
    # 删除样本
    def del_sample(self):
        QMessageBox.information(self.view, '删除样本', '不允许删除标注的样本。')
        self.cancel_select()
        return
        # 删除状态
        if self.status_annotate:
        ...
        if label[4] == 'None':
            ...
            label[4] = int(label[4])
        idx = self.sample_list.index(tuple(label))
        pick_channel, start, end, uid, type_id ,suid= label[0], label[1], label[2], label[3], label[4], label[6]
        # 只能删除当前用户标记的样本片段
        if self.stu_id == suid:
            msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id,suid]
            self.client.da_del_sample19(msg)
        else:
    ...
    def show_jump_sample_detail(self, sample):
        ...
        if type_name is None or type_name=='None':
            self.view.show_sample_detail('[未完成]', '', '', str(b_t), str(e_t), '', user_name)
            return
    ...
    def insert_update_state(self, type):
        ...
        if self.state_picked is not None and self.state_right is None:
            ...
            idx = self.sample_list.index(tuple(label))
            self.idx = idx
            begin, end, uid,suid= label[1], label[2], label[3], label[6]
            status = (pick_channel, begin, end, uid, type_id, type_name)
            s_label = '|'.join([str(x) for x in status[:]])
            msg = [self.check_id, self.file_id, begin, pick_channel, end, uid, type_id,self.class_id,suid]

            self.client.da_update_sampleInfo_state(msg)
        # 插入状态
        else:
            # begin, end = self.state_left, self.state_right
            # status = (pick_channel, begin, end, uid, type_id, type_name)
            # msg = [self.check_id, self.file_id, begin, pick_channel, end, uid, type_id]
            # self.client.da_add_sampleInfo_state(msg)
            QMessageBox.information(self, '不能添加状态样本',"不允许操作")
    def add_sampleInfo_stateRes(self, REPData):
        ...
        if REPData[1] == 22:
            ...
            idx = self.idx
            begin, end,uid,suid = label[1], label[2], label[3], label[6]
            status = (pick_channel, begin, end, uid, type_id, type_name,suid)
            s_label = '|'.join([str(x) for x in status[:]])
    ...
    # 删除状态
    def del_state(self):
        QMessageBox.information(self, '不能删除状态样本', "不允许操作")
        return False
        if self.state_picked is not None:
            ...
            msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id,self.class_id,suid]
            self.client.da_del_sampleInfo_state(msg)
    ...
    def on_btnSetting_clicked(self):
        ...
        self.settingView.ui.comboBoxChannel = CheckableComboBox(self.settingView.ui.gbSettings, channels, self.channels,
                                                                self.channels_name)
        self.settingView.ui.labelUserInfo.hide()
        self.settingView.ui.comboBoxUser.hide()
    ...
    def on_btnConfirm_clicked(self):
       ...
       msg = [self.check_id, self.file_id,self.patient_id,self.uid,self.class_id,self.stu_id]
       self.client.da_openEEGFile(msg)
    ...
    def timeChange(self):
        ...
        msg = [self.check_id, self.file_id, REQdata]
        self.client.da_load_dataDynamical10(msg)
    ...
    # 样本列表右键菜单-删除样本
    def del_label(self):
        QMessageBox.information(self.view, '删除样本', '不允许标注的样本。')
        self.cancel_select()
        return
        sample = self.rclick_pick_sample
        label = '|'.join([str(x) for x in sample[:]])
        ...
        pick_channel, start, end, uid, type_id,suid = sample[0], sample[1], sample[2], sample[3], sample[4], sample[6]
        if self.User[0] == suid:
            self.idx = idx
            msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id,self.class_id,suid]
            self.client.da_del_sampleInfo13(msg)
    ...
    def update_label(self, type):
        ...
        label = '|'.join([str(x) for x in sample[:]])
        idx = self.sample_list.index(sample)
        pick_channel, start, end,uid,suid = sample[0], sample[1], sample[2] ,sample[3],sample[6]
        ...
        try:
                ...
                msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id,self.class_id,suid]
                self.client.da_update_sampleInfo12(msg)

        except Exception as result:
                QMessageBox.information(self.view, '填写不成功', "失败原因: %s" % result)
        return
    ...
    def update_sample1415Res(self, REPData):
        ...
        type_id, type_name = self.p_type[0], self.p_type[1]
        cur_sample = (pick_channel, start, end, uid, type_id, type_name,suid)
        s_label = '|'.join([str(x) for x in cur_sample[:]])
