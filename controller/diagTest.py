...
from view.diagTest import DiagTestView
from view.diagTest import SettingView
...
class diagTestController(QWidget):
    ...
    def __init__(self, appUtil=None, Widget=None, client=None, mainMenubar=None, mainLayout=None):
        ...
        self.client.dt_get_type_infoResSig.connect(self.get_type_infoRes)
        self.client.dt_get_contentsResSig.connect(self.get_contentsRes)
        self.client.dt_openEEGFileResSig.connect(self.openEEGFileRes)
        self.client.dt_load_dataDynamicalResSig.connect(self.load_dataDynamicalRes)
        self.client.dt_update_sampleInfo12ResSig.connect(self.update_sampleInfo12Res)

        self.client.dt_del_sampleInfo13ResSig.connect(self.del_sampleInfo13Res)
        self.client.dt_add_sampleInfo14ResSig.connect(self.update_sample1415Res)
        self.client.dt_update_sampleInfo15ResSig.connect(self.update_sample1415Res)
        self.client.dt_insert_sample17ResSig.connect(self.insert_sample17Res)
        self.client.dt_init_SampleListResSig.connect(self.init_SampleListRes)

        self.client.dt_del_sample19ResSig.connect(self.del_sample19Res)
        self.client.dt_add_sampleInfo_stateResSig.connect(self.add_sampleInfo_stateRes)
        self.client.dt_update_sampleInfo_stateResSig.connect(self.add_sampleInfo_stateRes)
        self.client.dt_del_sampleInfo_stateResSig.connect(self.del_sampleInfo_stateRes)
        ...
        self.client.dt_get_type_info(msg)
    ...
    def exit(self):
        ...
        self.client.dt_get_type_infoResSig.disconnect()
        self.client.dt_get_contentsResSig.disconnect()
        self.client.dt_openEEGFileResSig.disconnect()
        self.client.dt_load_dataDynamicalResSig.disconnect()
        self.client.dt_update_sampleInfo12ResSig.disconnect()

        self.client.dt_del_sampleInfo13ResSig.disconnect()
        self.client.dt_add_sampleInfo14ResSig.disconnect()
        self.client.dt_update_sampleInfo15ResSig.disconnect()
        self.client.dt_insert_sample17ResSig.disconnect()
        self.client.dt_init_SampleListResSig.disconnect()

        self.client.dt_del_sample19ResSig.disconnect()
        self.client.dt_add_sampleInfo_stateResSig.disconnect()
        self.client.dt_update_sampleInfo_stateResSig.disconnect()
        self.client.dt_del_sampleInfo_stateResSig.disconnect()
    ...
    def init_prentryView(self):
        ...
        self.view.ui.btnSignInfo.setText("完成测试")
        ...
        self.view.ui.labelRoleInfo.hide()
        self.view.ui.labelRole.hide()
    ...
    def get_contentsRes(self, REPData):
        ...
        self.diagListView.show()
        self.diagListView.init_table(self.diags_viewInfo,self.userNamesDict,self.student_stateDict,self.paitentNamesDict,
    ...
    def on_clicked_manual_query(self, diags_viewInfo, patient_name,trow):
        ...
        self.view.ui.btnSignInfo.setText(f"完成[{diags_viewInfo[1]}]全部测试")
        self.page = ['file_name']
        try:
            msg = [self.check_id, self.file_id, self.patient_id,self.uid, self.class_id, self.client.tUser[0]]
            self.client.dt_openEEGFile(msg)
    ...
    def init_SampleList(self):
        ...
        msg = [self.check_id, self.file_id, channels, type_names, user_names, status_show, self.class_id,self.client.tUser[0]]
        self.client.dt_init_SampleList(msg)
    ...
    def disp_SampleList_ReadInitData(self):
        ...
        self.client.dt_load_dataDynamical(msg)
        ...
        for i in range(len(channels)):
            ...
            for sample in samples:
                if sample[0] != label:
                    continue
                # 当前用户标的样本颜色
                color2 = 'blue' if sample[6] == self.User[0] else 'cyan'
    ...
    def restore_pre_sample_color(self, sample_index):
        if len(self.sample_list) > sample_index:
            ...
            if sample[0] != 'all' and not (
                ...
                color = 'blue' if sample[6] == self.User[0] else 'cyan'
    ...
    def update_sample(self, type):
        ...
        if suid == self.User[0]:
            try:
                ...
                self.client.dt_update_sampleInfo15(msg)
            ...
            #     self.client.dt_add_sampleInfo14(msg)
            # except Exception as result:
    ...
    def del_sample(self):
        ...
        if self.User[0] == label[3]:
            msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id,self.class_id,suid]
            self.client.dt_del_sample19(msg)
        else:
    ...
    def del_state(self):
        ...
        if self.state_picked is not None:
            ...
            msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id,self.class_id,suid]
            self.client.dt_del_sampleInfo_state(msg)
    ...
    def on_btnSignInfo_clicked(self):
        self.stopRolling()
        reply = QMessageBox.information(self, '提示', '请确认本课堂培训内容的所有标注都已完成。\n完成学习测试提交后将不能操作或查看。确定吗？', QMessageBox.Yes | QMessageBox.No)
        if reply == 16384:
            msg = [self.class_id, self.client.tUser[0]]
            self.client.dt_diagTest_commit(msg)
    ...
    def timeChange(self):
        ...
        msg = [self.check_id, self.file_id, REQdata]
        self.client.dt_load_dataDynamical10(msg)
    ...
    def del_label(self):
        ...
        if self.User[0] == suid:
            self.idx = idx
            msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id,self.class_id, suid]
            self.client.dt_del_sampleInfo13(msg)
        else:
            QMessageBox.information(self.view, '删除样本', '不允许删除其他用户标注的样本。')
            self.cancel_select()
            return
    ...
    def update_label(self, type):
        ...
        s_label = '|'.join([str(x) for x in cur_sample[:]])
        # 如果选择样本为本用户标记，则修改，否则添加
        if suid == str(self.User[0]):
            try:

                msg = [self.check_id, self.file_id, start, pick_channel, end, uid,type_id,self.class_id,suid]
                self.client.dt_update_sampleInfo12(msg)
            ...
            # try:
            #     msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id]
            #     self.client.dt_add_sampleInfo11(msg)
            #
            # except Exception as result:
            #     QMessageBox.information(self.view, '添加失败', "失败原因: 当前位置已存在多个样本")
            return
