...
class DiagLearningView(QWidget):
...
class diagListView(QMainWindow,QWidget):
    def __init__(self,parent=None):
        super().__init__(parent)
        self.ui = Ui_diagList()
        self.ui.setupUi(self)
    ...
    def init_table(self, diags_viewInfo,curUser,userNamesDict,paitentNamesDict,on_clicked_manual_query,on_clicked_diag_query,on_clicked_learn_query=None):
        try:
            self.setGeometry(QRect(100, 84, 1608, 880))
            self.ui.tableWidget.clear()
            self.ui.tableWidget.setGeometry(QRect(5, 5, 1586, 870))
            ...
            self.ui.tableWidget.setHorizontalHeaderLabels(
                ["班级名称",'状态', '学习说明', '起始时间', '截止时间', '时长','检查单号', '文件号', '操作'])
            ...
            for row in range(0, self.row_num):
              if cur_class_id is None or cur_class_id!=diags_viewInfo[row][0]:
                ...
                font.setPointSize(12)
                i = 1
                self.ui.tableWidget.setItem(row, i, QTableWidgetItem(ltip[diags_viewInfo[row][-1]]))
                self.ui.tableWidget.item(row, i ).setTextAlignment(Qt.AlignCenter)
                elf.ui.tableWidget.item(row, i).setFlags(Qt.ItemIsEditable)
                font = self.ui.tableWidget.item(row, i ).font()
                font.setPointSize(12)
                i=2
              ...
              learnInfoBtn = QPushButton('我的学习记录')
              learnInfoBtn.clicked.connect(partial(on_clicked_learn_query, diags_viewInfo[row]))
              learnInfoBtn.setStyleSheet('height : 50px;font : 18px;color:blue')
