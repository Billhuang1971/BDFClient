...
from view.consulting import ConsultingView, BdfSettingView
from view.consulting import SettingView
from view.consulting import PrentryView
from view.consulting import sign_InfoView
from view.consulting import diagListView
...
class consultingController(QWidget):
    ...
    def __init__(self, appUtil=None, Widget=None, client=None, mainMenubar=None, mainLayout=None):
        super().__init__()
        self.view = ConsultingView()
        self.mainLayout = mainLayout
        self.mainMenubar = mainMenubar
        self.appUtil = appUtil
        self.client = client
        self.User = self.client.tUser
        self.client.ct_main_get_diagResSig.connect(self.ct_main_get_diagRes)
        self.client.ct_diag_commitResSig.connect(self.diag_commitRes)
        self.client.ct_diag_refusedResSig.connect(self.diag_refusedRes)
        self.client.ct_diag_updateResSig.connect(self.diag_updateRes)
        self.client.ct_get_diagResSig.connect(self.get_diagRes)
        self.client.ct_get_diags_notDiagResSig.connect(self.get_diags_notDiagRes)

        self.client.ct_get_type_infoResSig.connect(self.get_type_infoRes)

        self.client.ct_get_fileNameByIdDateResSig.connect(self.get_fileNameByIdDateRes)
        self.client.ct_openEEGFileResSig.connect(self.openEEGFileRes)
        self.client.ct_load_dataDynamicalResSig.connect(self.load_dataDynamicalRes)
        self.client.ct_update_sampleInfo12ResSig.connect(self.update_sampleInfo12Res)

        self.client.ct_del_sampleInfo13ResSig.connect(self.del_sampleInfo13Res)
        self.client.ct_add_sampleInfo14ResSig.connect(self.update_sample1415Res)
        self.client.ct_update_sampleInfo15ResSig.connect(self.update_sample1415Res)
        self.client.ct_insert_sample17ResSig.connect(self.insert_sample17Res)
        self.client.ct_init_SampleListResSig.connect(self.init_SampleListRes)

        self.client.ct_del_sample19ResSig.connect(self.del_sample19Res)
        self.client.ct_add_sampleInfo_stateResSig.connect(self.add_sampleInfo_stateRes)
        self.client.ct_update_sampleInfo_stateResSig.connect(self.add_sampleInfo_stateRes)
        self.client.ct_del_sampleInfo_stateResSig.connect(self.del_sampleInfo_stateRes)
        ...
        self.client.ct_get_type_info(msg)
    ...
    def exit(self):
        self.stopRolling()
        self.client.ct_main_get_diagResSig.disconnect()
        ...
        self.client.ct_get_type_infoResSig.disconnect()

        self.client.ct_get_fileNameByIdDateResSig.disconnect()
        self.client.ct_openEEGFileResSig.disconnect()
        self.client.ct_load_dataDynamicalResSig.disconnect()
        self.client.ct_update_sampleInfo12ResSig.disconnect()

        self.client.ct_del_sampleInfo13ResSig.disconnect()
        self.client.ct_add_sampleInfo14ResSig.disconnect()
        self.client.ct_update_sampleInfo15ResSig.disconnect()
        self.client.ct_insert_sample17ResSig.disconnect()
        self.client.ct_init_SampleListResSig.disconnect()

        self.client.ct_del_sample19ResSig.disconnect()
        self.client.ct_add_sampleInfo_stateResSig.disconnect()
        self.client.ct_update_sampleInfo_stateResSig.disconnect()
        self.client.ct_del_sampleInfo_stateResSig.disconnect()
    ...
    def on_clicked_manual_query(self, diags_viewInfo,patient_name):
        ...
        if self.uid==self.client.tUser[0]:
            self.view.ui.btnSignInfo.setText(
                f"[医生{self.userNamesDict.get(diags_viewInfo[2])},单号{diags_viewInfo[-2]}]填写诊断")
        else:
            self.view.ui.btnSignInfo.setText(
                f"[医生{self.userNamesDict.get(diags_viewInfo[2])},单号{diags_viewInfo[-2]}]查看诊断")
    ...
    def on_clicked_diag_query(self, diags_viewInfo):
        ...
        self.client.ct_main_get_diag(msg)
    ...
    def on_btnConfirm_clicked(self):
        try:
            ...
            msg = [self.check_id, self.file_id,self.patient_id,self.userNamesDict.get(self.uid)]
            self.client.ct_openEEGFile(msg)
            self.prentryView.hide()
    ...
    def init_SampleList(self):
        ...
        user_names  = '("{}")'.format(self.userNamesDict.get(self.uid))
        ...
        self.client.ct_init_SampleList(msg)
    ...
    def disp_SampleList_ReadInitData(self):
        ...
        self.client.ct_load_dataDynamical(msg)
    ...
    def paint_channels(self, data, begin: int, end: int, window_samples, channels=[]):
        ...
        if len(self.dgroup) == 0 or len(self.dgroup) >0 and self.mont_idx==0:
        ...
        #  当前窗口的样本
            ...
            for i in range(len(channels)):
                index = self.channels_default.index(channels[i])
                label = channels[i]
                if '-' in label:
                    g1, g2 = label.split('-')
                    try:
                        #g1_index = self.channels_name.index(g1)
                        g1_index = self.channels_default.index(g1)
                    ...
                    if g2 == 'REF':
                        ...
                        try:
                            ...
                            g2_index = self.channels_default.index(g2)
    ...
    def paint_sample(self, data, start, end, channel, color, label):
        ...
        if "'-'" in channel:
            ...
            index = self.channels_default.index(channel)
            y = data[index, s:e]
    ...
    def insert_sample(self, type):
        ...
        try:
            ...
            self.client.ct_insert_sample17(msg)
    ...
    def update_sample(self, type):
        ...
        if label[3] == self.User[0]:
            try:
                msg = [ self.check_id, self.file_id, start,pick_channel, end, uid, type_id]
                self.client.ct_update_sampleInfo15(msg)
            except Exception as result:
                QMessageBox.information(self.view, '添加失败', "失败原因: %s" % result)
        else:
            try:
                msg = [self.check_id,  self.file_id, start,pick_channel, end, uid, type_id]
                self.client.ct_add_sampleInfo14(msg)
            except Exception as result:
    ...
    def del_sample(self):
        ...
        if self.User[0] == label[3]:
            msg = [self.check_id, self.file_id, start,pick_channel, end, uid, type_id]
            self.client.ct_del_sample19(msg)
        else:
    ...
    def show_jump_sample_detail(self, sample):
        ...
        if sample[0] != 'all':
            ...
            if "'-'" in sample[0]:
                ...
                idx = self.channels_default.index(sample[0])
                y = self.data[idx, s:e]
            channel = sample[0]
            if len(y) > 0:
                m = abs(y.max() - y.min())
    ...
    def insert_update_state(self, type):
        ...
        if self.state_picked is not None and self.state_right is None:
            ...
            self.client.ct_update_sampleInfo_state(msg)
        # 插入状态
        else:
            begin, end = self.state_left, self.state_right
            status = (pick_channel, begin, end, uid, type_id, type_name)
            msg = [self.check_id, self.file_id, begin, pick_channel, end, uid, type_id]
            self.client.ct_add_sampleInfo_state(msg)
    ...
    def del_state(self):
        if self.state_picked is not None:
            ...
            label[4] = int(label[4])
            if self.User[0] != label[3]:
                QMessageBox.information(self.view, '删除样本', '不允许删除其他用户标注的状态样本。')
                self.cancel_select()
                return
            self.stopRolling()
            self.view.ui.btnShowInfo.setDisabled(True)
            ...
            msg = [self.check_id, self.file_id, start, pick_channel, end,uid, type_id]
            self.client.ct_del_sampleInfo_state(msg)
    ...
    def on_btnSetBdfExit_clicked(self):
        self.BdfSettingView.close()

    def on_btnConfirmSetBdf_clicked(self):
        ...
        if self.BdfSettingView.ui.radioButtonMontage_2.isChecked():
            self.montage="双极导联"
            self.mont_idx=1
        else:
            self.montage = "默认"
            self.mont_idx=0
        self.channels=[]
        for i in range(len(self.BdfSettingView.ui.ck_g)):
            if self.BdfSettingView.ui.ck_g[i].isChecked():
                self.channels.append(self.BdfSettingView.ui.ck_g[i].text())
    ...
    def on_btnSetting_clicked(self):
        if self.duration is None:
            return
        self.stopRolling()
        if len(self.dgroup)>0:
            self.BdfSettingView = BdfSettingView(self.mont_idx, self.dgroup, self.channels)

            self.BdfSettingView.ui.pb_ok.clicked.connect(self.on_btnConfirmSetBdf_clicked)
            self.BdfSettingView.ui.pb_cancel.clicked.connect(self.on_btnSetBdfExit_clicked)

            self.BdfSettingView.show()
            return

        type_name = [x[1] for x in self.type_info]
        #user_name = [x[3] for x in self.user_info]
        user_name =[self.userNamesDict.get(self.uid)]
    ...
    def timeChange(self):
        ...
        msg = [self.check_id, self.file_id, REQdata]
        self.client.ct_load_dataDynamical10(msg)
    ...
    def del_label(self):
        ...
        if self.User[0] == sample[3]:
            self.idx = idx
            msg = [self.check_id,self.file_id, start, pick_channel,end, uid, type_id]
            self.client.ct_del_sampleInfo13(msg)
    ...
    def update_label(self, type):
        ...
        if sample[3] == self.User[0]:
            try:
                ...
                msg = [self.check_id, self.file_id, start, pick_channel, end, uid, type_id]
                self.client.ct_update_sampleInfo12(msg)
            ...
            try:
                msg = [self.check_id, self.file_id, start,pick_channel, end, uid, type_id]
                self.client.ct_add_sampleInfo11(msg)
    ...
    #诊断学习/提取诊断信息回调
    def ct_main_get_diagRes(self, REPData):
        ...
        if REPData[0] == '0':
            ...
            return False
        d = REPData[2][0]
        msg=f"检查单号:{d[-2]},病人：{self.paitentNamesDict.get(d[0])},测量时间:{d[1]},诊断医生：{self.userNamesDict.get(d[2])},诊断时间：{d[4]}\n"
