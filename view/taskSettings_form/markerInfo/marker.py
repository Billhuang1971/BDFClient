...
class MarkerTableWidget(QWidget):
    control_signal = pyqtSignal(list)

    def __init__(self, *args, **kwargs, ):
        super(MarkerTableWidget, self).__init__(*args, **kwargs)
    ...
    def initUI(self):
        ...
        # 选择标注人员部分
        gLayout.addWidget(self.createMarkerInfoGroup())
        # 已经选择的标注人员
        gLayout.addWidget(self.createSelectMarkerGroup())
    ...
    # 初始化标注人员信息显示部分
    def createMarkerInfoGroup(self):
        groupBox = QGroupBox('可选择的标注人员', self)
        # groupBox.setFont(QFont('', 12))
        groupBox.setLayout(self.markerInfolayout)
        return groupBox


    # 已近选择的标注人员
    def createSelectMarkerGroup(self):
        groupBox = QGroupBox('已经选择的标注人员', self)
        groupBox.setFont(QFont('', 12))
        # 总体布局
        self.selectvLayout = QVBoxLayout(groupBox)
        # 表格布局
        self.selectVInfo = QVBoxLayout()
        # 按钮布局
        self.selectHorizontalLayout = QHBoxLayout()

        # 添加按钮
        self.selectHorizontalLayout.setObjectName("horizontalLayout")
        spacerItem = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
        self.selectHorizontalLayout.addItem(spacerItem)
        self.btnConfirm = QtWidgets.QPushButton()
        self.btnConfirm.setObjectName("btnConfirm")
        self.btnConfirm.setText("确认")
        self.btnConfirm.setFont(QFont('', 12))
        self.selectHorizontalLayout.addWidget(self.btnConfirm)
        self.btnReturn = QtWidgets.QPushButton()
        self.btnReturn.setObjectName("btnReturn")
        self.btnReturn.setFont(QFont('', 12))
        self.btnReturn.setText("返回")
        self.selectHorizontalLayout.addWidget(self.btnReturn)
        spacerItem1 = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
        self.selectHorizontalLayout.addItem(spacerItem1)

        self.selectvLayout.addLayout(self.selectVInfo)
        self.selectvLayout.addLayout(self.selectHorizontalLayout)
        self.selectvLayout.setStretch(0, 9)
        self.selectvLayout.setStretch(9, 10)

        groupBox.setLayout(self.selectvLayout)

        return groupBox

    # 已经选择的人员信息
    def init_selectedTable(self, col_label=None, sampleList=None, on_clicked_deleteBtn=None,):

        self.tableRow_selected = len(sampleList)
        self.tableCol_selected = len(col_label)
        self.col_label_selected = col_label
        self.sampleList_selected = sampleList
        # self.current_page = current_page
        # self.totalNum = totalNum
        self.selectedtable = QTableWidget(self.tableRow_selected, self.tableCol_selected)
        self.selectedtable.setHorizontalHeaderLabels(self.col_label_selected)

        # print(self.col_label)
        for row in range(0, self.tableRow_selected):
            for col in range(0, self.tableCol_selected-1):
                if isinstance(self.sampleList_selected[row][col+1], int):
                    self.text_item_selected = QTableWidgetItem(str(self.sampleList_selected[row][col+1]))
                else:
                    self.text_item_selected = QTableWidgetItem(self.sampleList_selected[row][col+1])
                self.text_item_selected.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                self.text_item_selected.setFont(QFont('', 12))
                self.selectedtable.setItem(row, col, self.text_item_selected)

                # 设置最后一列信息
                layout = QHBoxLayout()
                self.selectedtable.setCellWidget(row, self.tableCol_selected - 1, QWidget())
                deleteBtn = QPushButton('删除')
                deleteBtn.clicked.connect(partial(on_clicked_deleteBtn, row))
                deleteBtn.setStyleSheet('border: none;color:blue')
                deleteBtn.setFont(QFont('', 12))
                deleteBtn.setCursor(Qt.PointingHandCursor)
                layout.addWidget(deleteBtn)
                self.selectedtable.cellWidget(row, self.tableCol_selected - 1).setLayout(layout)
            self.selectedtable.setRowHeight(row, 50)

        self.selectedtable.horizontalHeader().setStyleSheet(
            '''font: 20px;border:none;border-bottom:1px solid rgb(210, 210, 210)''')
        self.selectedtable.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)  # 自适应宽度
        self.selectedtable.horizontalHeader().setDefaultSectionSize(200)
        self.selectedtable.horizontalHeader().setSectionResizeMode(2, QHeaderView.Interactive)
        self.selectedtable.setEditTriggers(QAbstractItemView.NoEditTriggers)

        # 不显示电话和邮箱两列
        self.selectedtable.setColumnHidden(1, True)
        self.selectedtable.setColumnHidden(2, True)

        self.selectVInfo.addWidget(self.selectedtable)
    ...
    def addOther(self):
        ...
        # 中心组件
        self.markerInfolayout = QVBoxLayout()

        # 添加搜索框以及查询按钮
        self.markerInfolayout.addLayout(self.horizontalLayout)

        # 添加表格
        self.verticalLayout_1 = QtWidgets.QVBoxLayout()
        # self.verticalLayout_1.addWidget(self.table)
        self.markerInfolayout.addLayout(self.verticalLayout_1)
    ...
    def setPageController(self, page):
        ...
        self.control_layout.addStretch(1)
        self.markerInfolayout.addLayout(self.control_layout)
