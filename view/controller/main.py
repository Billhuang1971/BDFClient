from assessLabel import assessLabelController
from auto import autoController
from clearLabel import clearLabelController
...
class MainController(QWidget):
    ...
    def signalConnection(self):
        ...
        self.view.ui.action_UC13.triggered.connect(lambda: self.view.setPosition('日常/远程诊断', '清理标注'))
        self.view.ui.action_UC13.triggered.connect(lambda: self.switch_page("clearLabelController"))
        ...
        self.view.ui.action_UC24.triggered.connect(lambda: self.switch_page("detailLookController"))
        ...
        self.view.ui.action_UC28.triggered.connect(lambda: self.view.setPosition('脑电科研支撑', '模型训练/测试'))
        self.view.ui.action_UC28.triggered.connect(lambda: self.switch_page("modelTrainController"))

        self.view.ui.action_UC29.triggered.connect(lambda: self.view.setPosition('脑电科研支撑', '脑电扫描'))
        self.view.ui.action_UC29.triggered.connect(lambda: self.switch_page("autoController"))

        self.view.ui.action_UC30.triggered.connect(lambda: self.view.setPosition('脑电科研支撑', '评估标注'))
        self.view.ui.action_UC30.triggered.connect(lambda: self.switch_page("assessLabelController"))

    # 用户切换
    def userSwitch(self):
        if 'diagLearningController' in self.m_controllers.keys():
            ...
            if studyInfo is not None and studyInfo != '':
                reply = QMessageBox.information(self, '提示',
                                                ...
                                                QMessageBox.Yes)
                return
        reply = QMessageBox.information(self, '提示', '是否需要注销，并登录其他账户', QMessageBox.Yes | QMessageBox.No)
        if reply == 16384:
    ...
    def logoutRes(self, REPData):
        if REPData[0] == '1':
            ...
            if 'diagLearningController' in self.m_controllers.keys():
                self.m_controllers['diagLearningController'].study_end()

            for contr in self.m_controllers.keys():
                self.m_controllers[contr].exit()
    ...
    def switch_page(self, controller_name):
        ...
        if controller_name not in self.m_controllers.keys():
            ...
            if self.previous_controller == controller_name:
                if self.controller is not None:
                    self.controller.exit()
                self.generate_controller(controller_name)
            else:
                if controller_name == "manualController":
                    self.controller = self.m_controllers[controller_name]
                    self.previous_controller = controller_name
                    if self.controller.check_id is None or self.controller.file_id is None:
                        self.controller.exit()
                        self.generate_controller(controller_name)
                    ...
                    self.controller = self.m_controllers[controller_name]
                    self.previous_controller = controller_name
                    if self.controller.model_id is None:
                        self.controller.exit()
                        self.generate_controller(controller_name)
                else:
                    self.controller = self.m_controllers[controller_name]
    ...
    def generate_controller(self, controller_name):
        if controller_name == "LoginController":
            ...
            self.controller = diagTestController(appUtil=self.cAppUtil, Widget=self.view.label_4, client=self.client)
            self.m_controllers[controller_name] = self.controller
        elif controller_name == "diagAssessController":
            self.controller = diagAssessController(appUtil=self.cAppUtil, Widget=self.view.label_4, client=self.client)
            self.m_controllers[controller_name] = self.controller
        elif controller_name == "reserchingController":
            self.controller = reserchingController(appUtil=self.cAppUtil, Widget=self.view.label_4, client=self.client)
            self.m_controllers[controller_name] = self.controller
